var enableZoomHandlers, disableZoomHandlers = undefined;
...
function Neod3Renderer() {
    ...
    function render(id, $container, visualization) {
        ...
        enableZoomHandlers = function () {
			if (ENABLE_ZOOMHANDLERS === false) {
				return;
			}
			//console.log("enable");
        ...
        function legend(svg, styles) {
          ...
          circles.attr({
            cy: function(node) {
              return (keys.indexOf(node)+1)*3.2*r+30;
            },
            fill: function(node) {
          ...
          text.text(function(node) {
            ...
            return label ? label.substring(5) : "";
              ...
              return (keys.indexOf(node)+1)*3.2*r+36;
          });
		  
		  svg.selectAll('g.relationship').data(keys);
...
*/
        ...
        var graphView = neo.graphView()
            .style(styleSheet)
            .width($container.width()).height($container.height()).on('nodeClicked', dummyFunc).on('relationshipClicked', dummyFunc);
        var svg = d3.select("#" + id).append("svg");
        var renderer = svg.data([graphModel]);
        legend(svg,existingStyles);
        var zoomHandlers = {};
        var zoomBehavior = d3.behavior.zoom().on("zoom", applyZoom).scaleExtent([0.2, 1]);
		...
		ENABLE_ZOOMHANDLERS = false;
        disableZoomHandlers();
		//enableZoomHandlers();
		setTimeout(function () {
			ENABLE_ZOOMHANDLERS = true;
			enableZoomHandlers();
		}, 100);
        ...
        return  {
			...
			'renderer': renderer,
			'update': function () {
				graphModelLinks = graphModel.relationships();
				for (var _l in graphModelLinks) {
					graphModelLinks[_l].type = "aaaa";
				}
