...
if (typeof(ENABLE_ZOOMHANDLERS) === "undefined") {
	var ENABLE_ZOOMHANDLERS = false;
}

neo.viz = function(el, graph, layout, style) {
  var clickHandler, force, geometry, onNodeClick, onNodeDblClick, onRelationshipClick, render, viz, onRelationshipMouseover, onNodeMouseover, onNodeMouseout;
  viz = {
    style: style
  };
  el = d3.select(el);
  ...
  onNodeMouseover = (function(_this) {
      return function(node) {
		  disableZoomHandlers();
		node_mouseover_event(node, this);
      return viz.trigger('nodeMouseover', node);
    };
  })(this);
  
  onNodeMouseout = (function(_this) {
      return function(node) {
		  enableZoomHandlers();
  ...
  onRelationshipClick = (function(_this) {
    return function(relationship) {
		relationship_click_event(relationship);
      return viz.trigger('relationshipClicked', relationship);
    };
  })(this);
  onRelationshipMouseover = (function(_this) {
	  var _link = _this;
    return function(relationship) {
		relationship_mouseover_event(relationship, this);
      return viz.trigger('relationshipMouseover', relationship);
    };
  })(this);
  ...
  viz.update = function() {
    ...
    relationshipGroups.enter().append("g").attr("class", "relationship").on("click", onRelationshipClick).on("mouseover", onRelationshipMouseover);
	el.select("g.layer.relationships").selectAll("g.relationship").append("title").text("");
	...
	el.select("g.layer.nodes").selectAll("g.node").append("title").text("");
	el.select("g.layer.nodes").selectAll("g.node").on("mouseover", onNodeMouseover).on("mouseout", onNodeMouseout);
...
(function() {
  ...
  nodeCaption = new neo.Renderer({
    onGraphChange: function(selection, viz) {
      ...
      text.html(function(line) {
        //return line.text;
		var _text_parts = line.text.split("(");
		if (_text_parts.length === 1) {
			return line.text;
		}
		else {
			return '<tspan x="0" dy="-0.8em">' + _text_parts[0] + '</tspan>'
				+ '<tspan x="0" dy="1.2em">' + "(" + _text_parts[1] + '</tspan>';
		}
